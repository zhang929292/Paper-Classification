...
class GCN( nn.Module ):
    def __init__(self,
        ...
        # input layer 1
        self.network_c1 = GraphConv(in_feats, n_hidden, activation=activation)
        # input layer 2
        self.network_s1 = GraphConv(in_feats, n_hidden, activation=activation)
        # hidden layer
        self.network_c2= GraphConv(n_hidden, n_hidden, activation=activation)
        self.network_s2 = GraphConv(n_hidden, n_hidden, activation=activation)
        # output layer
    ...
    def forward(self, features):
        x = features
        x1 = self.network_c3(self.g,self.dropout(self.network_c2(self.g,self.network_c1(self.g,x))))
        x2 = self.network_s3(self.g,self.dropout(self.network_s2(self.g,self.network_s1(self.g,x))))
        # h = layer(self.g, x)
        return x1, x2
...
def train(n_epochs=100, lr=1e-2, weight_decay=5e-4, n_hidden=16, n_layers=1, activation=F.relu , dropout=0.5,):
    ...
    for epoch in range(n_epochs):
        ...
        w2 = 0.1
        total_loss = w1 * (loss1.mean()+loss2.mean()) + w2 * (loss_gce.mean())
        optimizer.zero_grad()
...
if __name__ == '__main__':
    '''
    This is an improvement to the model structure, using
    The following ideas are employed in the loss:
    loss adds gce loss + double branch network structure improvement + sample weighting
