...
class GCN( nn.Module ):
    def __init__(self,
        ...
        # input layer 1. No input layer 2 and hidden layer
        self.network_in = GraphConv(in_feats,
                                    n_hidden, activation=activation)
        # output layer
        self.network_c3 = GraphConv(n_hidden,
                                    n_classes, activation=activation)
...
def train(n_epochs=100, lr=1e-2, weight_decay=5e-4, n_hidden=16, n_layers=1, activation=F.relu , dropout=0.5,):
    ...
    for epoch in range(n_epochs):
        ...
        loss_gce = bias_criterion(logits[train_mask], labels[train_mask])
        w3 = 0.1
        total_loss = loss + w3 * (loss_gce.mean())
        optimizer.zero_grad()
...
if __name__ == '__main__':
    '''
    Changed the network structure  
    Add gce loss + single-layer network structure + sample weighting 
    '''
    seed_num = 0
    algorithm = 'GCN9'
