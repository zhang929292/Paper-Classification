...
class GCN(nn.Module):
    ...
    def forward(self, features):
        x = features
        x1 = self.network_c3(self.g, self.dropout(self.network_c1(self.g, x)))
        x2 = self.network_s3(self.g, self.dropout(self.network_s1(self.g, x)))
        # h = layer(self.g, x)
        return x1, x2
...
def train(n_epochs=100, lr=1e-2, weight_decay=5e-4, n_hidden=16, n_layers=1, activation=F.relu, dropout=0.5, ):
    ...
    for epoch in range(n_epochs):
        ...
        w2 = 0.1
        total_loss = w1 * (loss1 + loss2) + w2 * (loss_gce.mean())
        optimizer.zero_grad()
...
if __name__ == '__main__':
    '''
    Both CE loss and GCE loss are weighted
    adding GCE loss + Double branch network structure improvement + weight sample (failed)
    ...
    train(n_epochs=args.max_epoch, lr=args.lr, weight_decay=args.weight_decay, n_hidden=args.n_hidden,
